...
def my_reduce(input_stream, num_top_entries, output_stream):
    ...
    for input in input_stream:
        word = input.split()[1].split(',')
        if input.split()[0] not in proj_dict:
            proj_dict[input.split()[0]] = []
        proj_dict[input.split()[0]].append((word[0][1:],word[1][:-1]))
    for key, val in proj_dict.items():
        new_list = []
        new_list = sorted(val, key=lambda x: int(x[1]), reverse=True)
        proj_dict[key] = new_list[:num_top_entries]
    for key, val in sorted(proj_dict.items()):
        for item in val:
            output_stream.write(key + "\t("+item[0]+","+item[1]+")\n")
    pass
